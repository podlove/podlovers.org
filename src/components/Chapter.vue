<template>
  <div class="flex px-2 -mx-2 rounded-sm" :class="{'font-normal': active}" @mouseover="mouseOverHandler" @mouseleave="mouseLeaveHandler" data-test="tab-chapters--entry">
    <span class="cursor-pointer w-8 py-2 mr-2 flex items-center justify-center" @click="selectChapter()">
      <icon v-if="action.icon" :type="action.icon" :size="24" />
      <span v-else>{{ action.content }}</span>
    </span>

    <chapter-progress class="w-full whitespace-no-wrap"
      :chapter="chapter"
      :playtime="playtime"
      :ghost="ghost"
      progressColor="white"
